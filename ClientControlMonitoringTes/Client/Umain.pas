...
begin
  ...
  case r.command of
    ...
    begin
      ...
      begin
        if isExeRunning(vNetSetting.RemoteServerName, false) then
          CloseCurrentHandleApplication(vNetSetting.RemoteServerName);

        FRemoteServerCMD := TAppExecute.Create;
        FRemoteServerCMD.OnStartExecute := nil;
        FRemoteServerCMD.OnEndExecute := nil;

        FRemoteServerCMD.FExecFname := vNetSetting.RemoteServerName;
        sCommand := IntToStr(vNetSetting.RemotePort) + ' ' + IntToStr(vNetSetting.RemoteScreen);
