unit uTCPServer;
{<  Unit uTCPServer contain TTCPServer as ICS WSocketServer wrapper.
    This
    @author  Andy Sucipto
...
//    FLongXAddress: Longword;
  ...
  public
    ...
    function getClientState(ipaddress: string): Boolean;
    function getClientSum: Integer;
...
begin
  if WSockServer.State = wsClosed then begin
    ...
    FLogStat('Server Listening at port ' + aPort);
...
//  FLogStat(TimeStr + ': ' + 'Connection from ' + ipAdd + ':' + cCon.PeerPort);
  FLogStat(ipAdd + ' Connected ' + cCon.PeerPort);
end;
...
//    ', duration: ' + FormatDateTime('hh:nn:ss', Now - cCon.ConnectTime));
  ...
  FLogStat(ipAdd + ' Disconnecting, See you again ');
...
function TTCPServer.getClientState(ipaddress: string): Boolean;
var
  i: integer;
  cCon: TClientConnected;
begin
  ...
  begin
    ...
    if (cCon.PeerAddr = ipaddress) and (cCon.State = wsConnected) then
    begin
      Result := True;
      Break;
    end;
...
function TTCPServer.getClientSum: Integer;
begin
  Result := WSockServer.ClientCount;
end;
