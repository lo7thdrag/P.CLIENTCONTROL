...
type
  ...
  private
    ...
    function GetApp(const fileName: string): Boolean;
...
begin
  ...
  begin
    ...
    if exe.szExeFile = FApplicationName1 then
    begin
  ...
  begin
    ...
    begin
      LogMemo.Lines.Add('Kill ' + FApplicationName1);
    end
  end;
end;
...
begin
  ...
  begin
    ...
    if exe.szExeFile = FApplicationName2 then
    begin
      IDExe := exe.th32ProcessID;
      flag := True;
      Break
  ...
  begin
    ...
    if TerminateProcess(killer, 0) then
    begin
      LogMemo.Lines.Add('Kill ' + FApplicationName2);
    end
  end;
end;
...
begin
  if not GetApp(FApplicationName1) then
  begin
    FAppGame.Executes;
    LogMemo.Lines.Add('Run ' + FApplicationName1);
  end
  else
    LogMemo.Lines.Add('Can not Run, double load ' + FApplicationName1);
//  FAppGame.Executes;
//  LogMemo.Lines.Add('Run ' + FApplicationName1);
end;
...
begin
  if not GetApp(FApplicationName2) then
  begin
    FAppGame2.Executes;
    LogMemo.Lines.Add('Run ' + FApplicationName2);
  end
  else
    LogMemo.Lines.Add('Can not Run, double load ' + FApplicationName2);
...
function TMainForm.GetApp(const fileName: string): Boolean;
var
  connector, killer :THandle;
  stamped : LongBool;
  exe : TProcessEntry32;
...
begin
  ...
  begin
    ...
    if exe.szExeFile = fileName{vNetSetting.Application} then
    begin
      IDExe := exe.th32ProcessID;
      Result := True;
      Break
